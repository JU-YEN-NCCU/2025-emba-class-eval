...
WEB_APP_URL = "https://script.google.com/macros/library/d/11egfvTY7y5Mif8i2NefD5momRkCYuaceO0z6U4_Gz-1Q1-EVnYt4_UBA/3"
...
if my_group != "請選擇" and name and sid:
    ...
    if target != "請選擇組別":
        ...
        **🔍 用戶期待 Desirability**
        * 定義問題與架構以形成洞見（Insight）
        * 解決顯性與隱性的用戶需求
        * 有驗證需求的方法、再次確認用戶之所需
        
        **💰 商業存續性 Viability**
        * 考量整體商業可能性、包括供應鏈及相關策略
        * 市場導入的需求評估、含可能競爭者之其他解決方案
        * 可執行的產品成本或預算計畫評估
        
        **🛠️ 技術可行性 Feasibility**
        * 原型（Prototyping）驅動的早期測試和錯誤發現
        * 預先技術評估與開發以支撐量產上市時之所需
        * 以用戶體驗為前提的技術發展藍圖（Roadmap）
        """)
